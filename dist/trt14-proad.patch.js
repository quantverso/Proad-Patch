...
`.trim();
	...
	function getTextNodes(root) {
		...
		while (current) {
			if (current.nodeType === Node.TEXT_NODE) nodes.push(current);
	...
	function buildGeneratedInnerHtml(text, context) {
	...
	async function generateAssistant(state) {
		...
		try {
			...
			if (delimitedRegion) {
				...
				editor.focus();
				const nativeSelection = editor.document.$.getSelection();
				if (!nativeSelection) throw new Error("Seleção nativa do editor não encontrada.");
			...
			const createGeneratedElement = (html) => {
				let nativeRange;
				if (nativeInsertionRange) nativeRange = nativeInsertionRange.cloneRange();
				else {
					const selection = editor.getSelection();
				...
				const span = nativeDocument.createElement("span");
			...
			if (error instanceof DOMException && error.name === "AbortError") {
				try {
					...
					console.error("[TRT14 Assistente] Erro ao atualizar editor após cancelamento:", updateError);
				}
  ...
  `;
	...
	function sortAnnotations() {
		...
		items.sort((a, b) => {
			...
			const result = colorOrder[a.annotation.color] - colorOrder[b.annotation.color];
