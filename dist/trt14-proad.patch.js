...
(function () {
  ...
  var MODEL_CONFIG = {
    ...
    'gemini-3.5-flash': {
      ...
      description:
        'Modelo Flash de alta capacidade para geração de documentos.',
      thinking: ['auto', 'minimal', 'low', 'medium', 'high'],
  ...
  function requestToPromise(request) {
    return new Promise((resolve, reject) => {
      ...
      request.onerror = () =>
        reject(
          request.error ??
            /* @__PURE__ */ new Error('IndexedDB request failed.'),
        );
    });
  }
  function transactionToPromise(transaction) {
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = () =>
        reject(
          transaction.error ??
            /* @__PURE__ */ new Error('IndexedDB transaction failed.'),
        );
      transaction.onabort = () =>
        reject(
          transaction.error ??
            /* @__PURE__ */ new Error('IndexedDB transaction aborted.'),
        );
    });
  }
  function openDatabase() {
    ...
    dbPromise = new Promise((resolve, reject) => {
      ...
      request.onerror = () => {
        ...
        reject(
          request.error ??
            /* @__PURE__ */ new Error('Não foi possível abrir o IndexedDB.'),
        );
      };
      request.onblocked = () => {
        console.warn(
    ...
    if (nested && typeof nested === 'object' && !Array.isArray(nested))
      return {
        context: Object.fromEntries(
          Object.entries(nested).map(([key, item]) => [
            key,
            String(item ?? ''),
          ]),
        ),
...
`.trim();
  ...
  function getProadTitle() {
    return (
      document.querySelector('h1.tituloPagina')?.textContent?.trim() || null
    );
  }
  ...
  function resolvePosition(nodes, position) {
    ...
    if (nodes.length > 0) {
      if (
        position === nodes.reduce((total, node) => total + node.data.length, 0)
      ) {
  ...
  async function generateAssistant(state) {
    ...
    const input = buildPrompt(
      state,
      maskDocument(authorizedDocument, context),
      contextKeys,
      getProadNumber(),
      isDelimited,
    );
    ...
    try {
      ...
      setAssistantStatus(
        state,
        completed ? 'Concluído.' : 'Geração finalizada.',
      );
    } catch (error) {
  ...
  `;
    ...
    contextPanel
      ...
      ?.addEventListener('click', () => {
        ...
        renderEmptyContext(state);
        if (state.proadReference)
          clearProadContext(state.proadReference).catch((error) => {
  ...
  function getProcessKey(row) {
    const match = (row.dataset.rk || '').match(
      /Protocolo\s+(\d+)\s+ano\s+(\d+)/i,
    );
    if (match) return `${match[1]}/${match[2]}`;
  ...
  function applyAnnotationFilter() {
    ...
    for (const row of getRows()) {
      let match = true;
      if (term)
        match = normalizeForSearch(
          getAnnotation(getProcessKey(row)).text,
        ).includes(term);
  ...
  function createColumnButton() {
    const tableContainer = document.getElementById(
      'formProtocolos:tblEstouTratando',
    );
    if (!tableContainer || document.getElementById('proad-column-toggle'))
      return;
  ...
  function setupContextPanel(processKey, refs, initialContext) {
    ...
    document
      .querySelector('#proad-annotation-modal [data-action="add"]')
      ?.addEventListener('click', (event) => {
        ...
        createContextRow(refs, '', '', () => {
          ...
          .querySelector('.proad-context-input')
          ?.focus();
        persist();
      });
    document
      .querySelector('#proad-annotation-modal [data-action="clear"]')
      ?.addEventListener('click', async (event) => {
        ...
        try {
          ...
          console.error(
            '[TRT14 Estou Tratando] Erro ao limpar contexto:',
            error,
          );
  ...
  function setupTodoPanel(processKey, todoList, initialTodos) {
    ...
    document
      .querySelector('#proad-annotation-modal .proad-todo-add')
      ?.addEventListener('click', (event) => {
        event.preventDefault();
        event.stopPropagation();
        createTodoCard();
  ...
  `;
    ...
    const contextStatusText = overlay.querySelector(
    ...
    const saveAnnotationAndClose = async () => {
      ...
      const colorValue = overlay.querySelector(
        'input[name="proad-annotation-color"]:checked',
      )?.value;
      const color =
        colorValue && COLORS[colorValue] ? colorValue : DEFAULT_COLOR;
  ...
  function captureOriginalToolbarButtonColors() {
    const targetLabels = /* @__PURE__ */ new Set([
  ...
  function observePage() {
    ...
    bodyObserver = new MutationObserver((mutations) => {
      ...
      const relevantMutations = childListMutations.filter(
        isTableRelatedMutation,
      );
